...
size_t powerful_digit_sum(size_t limit)
{
    size_t max_sum = 0;
    size_t sum_base = 0;
    size_t sum_exp = 0;
    ...
    {
        for (size_t j = 2; j != limit; ++j)
        {
            size_t sum = 0;
            vector<size_t> digit_blocks = big_exp(i,j);
            for (const auto& block : digit_blocks)
            {
                vector<unsigned char> bdigits = get_rdigits(block);
                for (const auto& digit: bdigits)
                    sum += digit;
            }
            if (sum > max_sum)
            {
...
int main()
{
    ...
    size_t bob = powerful_digit_sum(100);
